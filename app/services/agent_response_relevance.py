...
def normalize_quality_review_language(text: str) -> str:
    ...
    replacements = (
        ...
        (
            r"这是[“\"]增收不增利[”\"]在毛利层面的直接信号",
            "这表示规模扩张与毛利率下降同时发生",
        ),
        (
            r"说明盈利转化成真金白银的速度在放缓",
            "经营现金流与净利润的增长速度不同，具体原因尚未确认",
        ),
        (
            r"现金流对利润的覆盖比率",
            "经营现金流与归母净利润的比率",
        ...
        (
            r"这很重要，因为利润表漂亮的时候，现金流的变化往往更能暴露"
            r"应收账款或存货占款的真实程度——公司现在只解释库存增加是为了备货，"
            r"并没有说明客户回款节奏是否在变慢。?",
            "公司解释库存增加是为下半年备货，但没有解释经营现金流与净利润"
            "增速差异的原因。",
        ),
        (
            r"当前毛利率下降和现金流转化率走弱可能指向同一个价格或成本压力源头",
            "当前毛利率下降与现金流增速差异的原因都尚未确认",
        ),
        (
            r"毛利率下滑是价格竞争、产品结构还是成本端的压力",
            "毛利率下滑的具体原因",
        ),
        (
