...
def _normalize_market_reassessment_language(
    ...
    if user_has_window is None:
        answer = re.sub(
            r"(?:未来|后续|接下来|后面)(?:的)?(?:几个|数个|若干个)交易日(?:里|中|内)?",
            "后续完整交易日里",
            answer,
        )
        answer = re.sub(
            r"(?:之后|此后)(?:的)?(?:几个|数个|若干个)(?:完整)?交易日(?:里|中|内)?",
            "后续完整交易日里",
            answer,
        )
        answer = re.sub(
        ...
        answer = re.sub(
            r"(?:接下来)?(?:一两|两三|三五|几|数)天(?:内|后)?",
            "后续完整交易日里",
            answer,
        )
    answer = re.sub(
        r"(?:20|二十)日(?:移动)?均线(?:代表|反映|相当于)(?:了)?"
        r"[^。；\n]{0,32}(?:平均持仓成本|平均成本)",
        "二十日均线是过去二十个交易日收盘价的滚动平均",
        answer,
    )
    answer = re.sub(
        r"二十日均线是过去(?:若干|几个|数个)(?:个)?交易日收盘价的滚动平均",
        "二十日均线是过去二十个交易日收盘价的滚动平均",
        answer,
    )
    answer = re.sub(
        r"(?:始终|一直)?被压在(?P<line>(?:这条)?(?:20|二十)日(?:移动)?均线|"
        r"这条均线|MA20)(?:下面|下方)",
        r"仍位于\g<line>下方",
        answer,
    )
