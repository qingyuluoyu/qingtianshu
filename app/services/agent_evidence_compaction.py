...
def compact_stock_research_evidence(
    ...
    if focus == "quality_review":
        ...
        if focused_quality_followup:
            ...
            frame = {
                ...
                "answer_contract": (
                    ...
                    "中文，以数字和财务含义推进，不用比喻、口号或报告标签。"
                ),
            }
            frame["fact_candidates"] = frame["fact_candidates"][:requested_fact_count]
            return {
        ...
        frame = {
            ...
            "answer_contract": (
                "写四个连贯自然段：先给整体判断并说规模增长，再解释毛利率和"
                "主营结构、现金流，最后合并存货与财务费用后自然收束。信息要充足"
                "但不逐项念指标；使用平实专业的中文，不使用标题、项目符号、比喻、"
                "口号、评价标签或固定核验清单。毛利率段保留整体毛利率两期值以及"
                "动力、储能两项主营毛利率变化。原因未确认时只说尚未确认，不举可能"
                "原因或替代情景。"
            ),
        }
        result = {
