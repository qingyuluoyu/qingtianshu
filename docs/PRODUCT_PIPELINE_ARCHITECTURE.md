...
flowchart TD
    ...
    INPUT --> CHECK{"必需数据是否完整"}
    CHECK -->|完整| RULES["每个历史日运行同一版9条候选规则\n只使用当日可得数据"]
    CHECK -->|明确缺口| INCOMPLETE["逐日 data_incomplete\n不进入候选并披露覆盖率"]
    RULES --> STATES["PostgreSQL 逐日策略状态\n候选 / 未满足 / 数据不完整"]
    INCOMPLETE --> STATES
    STATES --> CHANGE["每日更新最新候选集合"]
    CHANGE --> NEXTOPEN["最早在下一交易日开盘执行\n任意两次换仓至少间隔10个交易日"]
    NEXTOPEN --> EQUAL["冷却期只保留最新目标\n候选等资金配置；为空则持有现金"]
    EQUAL --> NAV["零交易成本组合净值\n年化收益、最大回撤、换手"]
    NAV --> BENCH["沪深300同暴露基准\n策略空仓时基准同步冻结"]
...
4. 候选信号和成交时间分离：收盘后得到的信号最早只允许在下一交易日开盘执行，避免同收盘价成交的前视偏差；任意两次实际换仓至少间隔10个交易日，冷却期内的新信号覆盖旧目标，不排队执行过时组合。
...
10. 历史日规则由逐日重复构造数据帧改为等价滚动计算；真实快照抽样逐项对比63日和756日结果均一致，63日抽样速度约提升13倍。后台单批规则核验提升到100只；单批最多补齐12只外部数据，但同时请求仍限制为4只，兼顾吞吐与数据源稳定性。
11. 三档回测按用户可见结果确定后台优先级：先补齐阻塞3个月发布的股票，再补1年，最后扩展3年；覆盖检查过程中才发现需要扩展历史窗口的股票，会在最终外部同步批次形成前重新按同一优先级排序，避免被后加入同步队列而失去短窗口优先权。
12. 历史市值提示日期只用于提供该日市值，不再作为行情查询截止日；日线、复权因子和涨跌停价始终查询到请求窗口末端。区间内停牌可延续最后收盘价，正常上市股票的尾部行情缺口必须标记为 `data_incomplete`，只有明确退市股票才使用最后可得复权收盘价作为强制退出代理。
13. `requested_as_of_date` 已从大体积 JSON 提升为数据库快照元数据。后台不加载整份逐股历史也能区分“已查询到窗口末端”和“最后实际交易日较早”，避免退市股票重复同步。上市前自然没有行情的交易日不计为覆盖缺口；从首个真实复权收盘价开始，后续任何 `data_incomplete` 状态都会把该股票计入缺口覆盖。上游明确返回数据缺口时，系统保存逐日 `data_incomplete` 状态并披露完整数据覆盖率，不让少数永久缺口堵住整个历史范围，也不把缺口股票伪装成通过规则的候选。
14. 李总策略首屏只返回紧凑的公开运行摘要，不再把数千只股票的内部数据版本映射发送到浏览器；覆盖率和规则漏斗共用同一次候选状态读取。真实生产数据下，候选摘要响应由约345KB降至约6KB，耗时由约6.07秒降至约2.04秒；浏览器直达页约2.69秒完成策略版本、数据日期、股票范围和回测进度加载。
15. 沪深300缓存按真实指数行日期验收，而不是相信请求元数据中的截止日。63、252、756日任一窗口缺少目标交易日都会重新同步并拒绝发布；基准只在策略持仓期保持同等市场暴露，策略空仓期同步冻结，进出场必须使用当日真实开盘价。
16. 最佳可得历史的判定使用市场历史版本和已查询截止日，不依赖 `sync_profile`。同一逐股快照被实时策略任务重新发布后，即使任务标签变化，长期停牌股票也不会重新进入无限扩展队列。
...
- 将交易成本作为可切换的压力测试情景；当前用户确认的主口径固定为零成本，并允许查看每次换仓的入选规则证据。
