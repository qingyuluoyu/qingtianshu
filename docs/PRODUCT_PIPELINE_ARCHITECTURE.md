...
12. 历史股票如果已经退市或长期停牌，使用历史全市场市值截面定位最后可用交易日，不再反复请求最近5日后永久卡在“数据不完整”；明确退市后以最后可得复权收盘价作为强制退出代理，并继续计入统一摩擦成本。
13. `requested_as_of_date` 已从大体积 JSON 提升为数据库快照元数据。后台不加载整份逐股历史也能区分“已查询到窗口末端”和“最后实际交易日较早”，避免退市股票重复同步。上游明确返回数据缺口时，系统保存逐日 `data_incomplete` 状态并披露完整数据覆盖率，不让少数永久缺口堵住整个历史范围，也不把缺口股票伪装成通过规则的候选。
